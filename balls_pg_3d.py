''' 
    ...
    updated for ball bouncing in 3-d space
    
    Notsoseamless  - January 2016
...
pygame.display.set_caption("3-D Balls")
...
def spawn_ball():
    ...
    # initialize ball_pos and ball_vel for new ball in middle of cube
    pos = [WIDTH / 2, HEIGHT / 2, DEPTH / 2]

    # randomize the velocity, assuming that the draw handler runs about 60 times each second
    vel = [0, 0, 0]

    forward_vector =  [random.randrange(-3.0, 3.0), random.randrange(-2.0, 2.0), random.randrange(-2.0, 2.0)] 

    BALL_GROUP.add(Ball(pos, BALL_RADIUS, vel, forward_vector)) 
...
class Ball:
    ''' basic ball class '''
    def __init__(self, pos, radius, vel, forward_vector):
        ''' init methos '''
        self._radius = radius
        self._pos = [pos[0], pos[1], pos[2]]
        self._vel = [vel[0], vel[1], pos[2]]
	    # velocity update
        ...
        self._vel[2] += self._forward_vector[2]

    def draw(self, canvas):
        ''' draw ball on canvas '''
        pygame.draw.circle(canvas, white_color, [int(self._pos[0]), 
                             int(self._pos[1])], calculate_radius(self._radius, self._pos[2]), 1)
    ...
    def bounce(self):
        ''' bounce the ball '''
        print 'Speed was ', self._vel[0], ' ', self._vel[1], self.vel[2]
        # reverse for now...
        ...
        print 'now',  self._vel[0], ' ', self._vel[1], self._vel[2]
...
def calculate_radius(radius, depth):
    ''' returns radius dependant on depth position
        higest value is closest to observer '''
    result = 0
    if depth >= DEPTH:
        result = radius
    elif depth < 0
